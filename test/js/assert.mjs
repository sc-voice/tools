...
describe('assert', () => {
  it('TESTTESTok', () => {
    ...
    let eCaught;
    try { Assert.ok(undefined, 'undefined?'); } catch(e) { eCaught = e; }
    should(eCaught.message).equal('undefined?');
    try { Assert.ok(null, 'null?'); } catch(e) { eCaught = e; }
    should(eCaught.message).equal('null?');
    try { Assert.ok(false, 'false?'); } catch(e) { eCaught = e; }
    should(eCaught.message).equal('false?');
    try { Assert.ok(0, '0?'); } catch(e) { eCaught = e; }
    should(eCaught.message).equal('0?');
    try { Assert.ok(Number('NaN'), 'NaN?'); } catch(e) { eCaught = e; }
